...
# --- DEPLOY_PATCH: config & static ---
import os
from flask_cors import CORS
from pathlib import Path
ROOT_DIR = Path(__file__).resolve().parents[2] if (Path(__file__).resolve().parents[2] / "index.html").exists() else Path(__file__).resolve().parents[1].parents[0]
# Serve static (site root) from project root
APP_ROOT = ROOT_DIR
...
def scrape():
    ...
    # 1) Fetch via Playwright (headless then headful fallback)
    try:
    ...
    # 2) Save to temp and call parser with --html_path
        ...
        if manifest_path.exists():
            ...
            if last:
                ...
                if fpath.exists():
                    ...
                    # augment with weapon_code from HTML if missing
